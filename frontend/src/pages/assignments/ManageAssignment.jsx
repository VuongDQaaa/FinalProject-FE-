...
export default function ManageAssignment() {
        ...
        respData.forEach((assignment) => {
          assignment.state =
            assignment.state === "WaitingForAcceptance"
              ...
              : assignment.state;
          assignment.assignedDate = moment(
          ...
          assignment.action = [
            ...
            >
              <Link
                style={{ color: "green" }}
                to={`/editAssignment/${assignment.id}`}
                id="editButton"
              >
  ...
  const finalData =
    date == null
      ...
      : filterData.filter(
          (u) =>
            u.assignedDate ===
            moment(new Date(date).toLocaleDateString("en-US")).format(
              "DD/MM/YYYY"
            )
        );
  ...
  const handleReturnOk = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_UNSPLASH_RETURNREQUEST_ADMIN}/${idCompleted}`
      )
      .then(() => {
        setIdCompleted(null);
