...
export default function EditAssignmentPage() {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_UNSPLASH_ASSIGNMENT}/${assignmentId}`)
      .then((response) => {
        const assignmentData = {
          assignToId: response.data.assignToId,
          fullName:
            response.data.assignToFirstname +
            " " +
            response.data.assignToLastname,
          assetName: response.data.assetName,
          assignedDate: moment.tz(
            response.data.assignedData,
            "Asia/Ho_Chi_Minh"
          ),
        ...
        footer={[
          <Button
            ...
            onClick={() => {
              ...
              setUserInputValue({
                ...userInputValue,
                userInputValue: submitData.fullName,
              });
          ...
          <Button
            ...
            disabled={userModal.isLoading === true}
            key="back"
            onClick={() => {
        ...
        footer={[
          <Button
            ...
            onClick={() => {
              ...
              setAssetInputValue({
                ...assetInputValue,
                assetInputValue: submitData.assetName,
              });
            ...
            >
              ...
              <Form.Item label="Assigned Date" style={{ marginBottom: 0 }}>
                <Form.Item
                  ...
                  rules={[
                    {
                      required: true,
                      message: "Assigned Date must be required",
                    },
                    (fieldvalue) => ({
                      validator(_, value) {
                        if (moment(new Date()).isAfter(value._d)) {
                          return Promise.reject(
                            "Assigned Date can not be in the past"
                          );
                        } else {
              ...
              <Form.Item label="Note" style={{ marginBottom: 40 }}>
                <Form.Item
                  name="note"
                  rules={[
                    { required: true, message: "Note must be required" },
